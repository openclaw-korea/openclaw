...
read_when: "다중 에이전트 게이트웨이에서 에이전트별 샌드박싱 또는 에이전트별 도구 허용/거부 정책을 원할 때"
status: active
---

# 다중 에이전트 샌드박스 및 도구 설정

## 개요

다중 에이전트 설정의 각 에이전트는 이제 다음을 가질 수 있습니다:

- **샌드박스 설정** (`agents.list[].sandbox` > `agents.defaults.sandbox` 재정의)
- **도구 제한** (`tools.allow` / `tools.deny`, 추가로 `agents.list[].tools`)

이를 통해 다양한 보안 프로필을 가진 여러 에이전트를 실행할 수 있습니다:

- 전체 액세스 권한이 있는 개인 어시스턴트
- 도구가 제한된 가족/업무 에이전트
- 샌드박스에서 실행되는 공개 에이전트

`setupCommand`는 `sandbox.docker` (전역 또는 에이전트별)에 속하며 컨테이너가 생성될 때 한 번 실행됩니다.
...
자격 증명은 에이전트 간에 **공유되지 않습니다**. `agentDir`을 에이전트 간에 재사용하지 마세요.
자격 증명을 공유하려면 `auth-profiles.json`을 다른 에이전트의 `agentDir`에 복사하세요.

샌드박싱의 런타임 동작에 대해서는 [샌드박싱](/gateway/sandboxing)을 참조하세요.
"이것이 차단된 이유는?"를 디버깅하려면 [샌드박스 vs 도구 정책 vs 상승](/gateway/sandbox-vs-tool-policy-vs-elevated) 및 `openclaw sandbox explain`을 참조하세요.
...
전역(`agents.defaults.*`)과 에이전트별(`agents.list[].*`) 설정이 모두 있을 때:
...
- `agents.list[].sandbox.{docker,browser,prune}.*`는 해당 에이전트에 대해 `agents.defaults.sandbox.{docker,browser,prune}.*`를 재정의합니다(샌드박스 범위가 `"shared"`로 해결될 때 무시됨).
...
8. **서브 에이전트 도구 정책** (`tools.subagents.tools`, 해당하는 경우)
...
`tools.elevated`는 전역 기본값입니다(발신자 기반 허용 목록). `agents.list[].tools.elevated`는 특정 에이전트에 대해 상승을 추가로 제한할 수 있습니다(둘 다 허용해야 함).

완화 패턴:

- 신뢰할 수 없는 에이전트에 대해 `exec` 거부(`agents.list[].tools.deny: ["exec"]`)
- 제한된 에이전트로 라우팅하는 발신자를 허용 목록에 추가하지 않기
- 전역적으로 상승 비활성화(`tools.elevated.enabled: false`) (샌드박스 실행만 원하는 경우)
- 에이전트별 상승 비활성화(`agents.list[].tools.elevated.enabled: false`) (민감한 프로필의 경우)
...
레거시 `agent.*` 설정은 `openclaw doctor`에 의해 마이그레이션됩니다. 앞으로는 `agents.defaults` + `agents.list`를 선호하세요.
...
그룹/채널 세션은 항상 자신의 키를 가지므로 non-main으로 처리되고 샌드박스됩니다. 에이전트가 절대 샌드박스되지 않으려면 `agents.list[].sandbox.mode: "off"`를 설정하세요.
...
- 에이전트별 설정이 우선하므로 `agents.list[].sandbox.mode: "all"`을 설정하세요

### 거부 목록에도 불구하고 도구가 여전히 사용 가능함

- 도구 필터링 순서 확인: 전역 → 에이전트 → 샌드박스 → 서브 에이전트
- 각 수준은 추가로만 제한할 수 있고 부여할 수는 없습니다
