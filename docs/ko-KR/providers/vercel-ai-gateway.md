...
read_when:
  - OpenClaw와 함께 Vercel AI Gateway를 사용하고 싶을 때
...
게이트웨이가 데몬(launchd/systemd)으로 실행되는 경우, `AI_GATEWAY_API_KEY`가 해당 프로세스에서 사용 가능한지 확인하세요(예: `~/.openclaw/.env` 또는 `env.shellEnv`를 통해).
