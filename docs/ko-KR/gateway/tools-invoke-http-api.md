---
summary: "게이트웨이 HTTP 엔드포인트를 통해 단일 도구를 직접 호출합니다"
read_when:
...
OpenClaw의 게이트웨이는 단일 도구를 직접 호출하기 위한 간단한 HTTP 엔드포인트를 제공합니다. 이 엔드포인트는 항상 활성화되어 있으며, 게이트웨이 인증과 도구 정책으로 제한됩니다.
...
- `tool` (문자열, 필수): 호출할 도구의 이름입니다.
- `action` (문자열, 선택): 도구 스키마가 `action`을 지원하고 args 페이로드에서 생략된 경우 args로 매핑됩니다.
- `args` (객체, 선택): 도구별 인수입니다.
- `sessionKey` (문자열, 선택): 대상 세션 키입니다. 생략하거나 `"main"`인 경우, 게이트웨이는 구성된 메인 세션 키를 사용합니다 (`session.mainKey` 및 기본 에이전트를 존중하거나, 전역 범위에서는 `global`).
- `dryRun` (부울, 선택): 향후 사용을 위해 예약됨; 현재는 무시됩니다.
